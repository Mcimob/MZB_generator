from pykml import parser
from pykml.factory import nsmap
import requests
import json
import openpyxl
import matplotlib.pyplot as plt
from wgs84_ch1903 import GPSConverter

def create_marker(index, title, east, north):
    return f"""<Placemark id="marker_{index + 1}">
      <ExtendedData>
        <Data name="type">
          <value>marker</value>
        </Data>
      </ExtendedData>
      <name>Marker {title}</name>
      <description></description>
      <Style>
        <IconStyle>
          <Icon>
            <href>https://api3.geo.admin.ch/color/255,0,0/marker-24@2x.png</href>
            <gx:w>48</gx:w>
            <gx:h>48</gx:h>
          </Icon>
          <hotSpot x="24" y="4.799999999999997" xunits="pixels" yunits="pixels"/>
        </IconStyle>
        <LabelStyle>
          <color>ff0000ff</color>
        </LabelStyle>
      </Style>
      <Point>
        <tessellate>1</tessellate>
        <altitudeMode>clampToGround</altitudeMode>
        <coordinates>{east},{north}</coordinates>
      </Point>
    </Placemark>"""

def addMarkersToKML(outFile, poi, template):
    markers = []
    for index, p in enumerate(poi):
        north, east, alt = (converter.LV03toWGS84(p['easting'], p['northing'], 0))
        marker = create_marker(index, index, east, north)
        markers.append(marker)
        
    
    index = template.find('</Document>')
    outFile.write(template[0:index])
    for m in markers:
        outFile.write(m)
    outFile.write(template[index:-1])
    return

with open("map2.kml", 'rb') as f:
    root = parser.parse(f).getroot()
    pms = root.xpath(".//ns:Placemark[.//ns:LineString]", namespaces={'ns': nsmap[None]})

coords = {"type": "LineString", "coordinates": []}
converter = GPSConverter()

for coord in pms[0].LineString.coordinates.text.split(' '):
    xy = coord.split(',')
    print(float(xy[0]), float(xy[1]))
    coords['coordinates'].append(converter.WGS84toLV03(float(xy[1]), float(xy[0]), 0, clip=True)[0:2])
    
coords = requests.get(f'https://api3.geo.admin.ch/rest/services/profile.json?geom={json.dumps(coords)}&sr=21781&distinct_points=True', timeout=3, ).json()

for p in coords:
    p['alt'] = p['alts']['DTM2']
    del p['alts']

for i in range(len(coords)):
    coords[i]['relative'] = 'Start' if i==0 else 'End' if i==len(coords)-1 else 'High' if (coords[i-1]['alt'] < coords[i]['alt'] and coords[i]['alt'] > coords[i+1]['alt']) else 'Low' if (coords[i-1]['alt'] > coords[i]['alt'] and coords[i]['alt'] < coords[i+1]['alt']) else None

poi_tmp = [c for c in coords if c['relative']]
poi = []
over = True
margin = 0
while over:
    over = False
    
    for i in range(1, len(poi_tmp) - 1):
        if max(abs(poi_tmp[i]['alt'] - poi_tmp[i-1]['alt']), abs(poi_tmp[i]['alt'] - poi_tmp[i+1]['alt'])) >= margin:
            poi.append(poi_tmp[i])

    poi.append(poi_tmp[-1])
    poi.insert(0, poi_tmp[0])

    if len(poi) > 21:
        over = True
        margin += 5
        poi = []
print(poi, margin)    

with open('map2.kml', 'r') as f:
    with open('out.kml', 'w') as out:
        addMarkersToKML(out, poi, f.read())

book = openpyxl.load_workbook('MZB_template.xlsx')
sheet = book['leer']

for i in range(len(poi)):
    sheet[f'A{i+8}'] = f'Point {i+1}'
    sheet[f'C{i+8}'] = poi[i]['alt']
    
    if i != 0:
        sheet[f'E{i+8}'] = (poi[i]['dist'] - poi[i-1]['dist'])/1000

book.save('MZB.xlsx')


x = [p['dist'] for p in coords]
y = [p['alt'] for p in coords]
fig, ax = plt.subplots()

ax.plot(x, y)

plt.show()
